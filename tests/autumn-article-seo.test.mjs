...
const schemaHasType = (schema, type) => Array.isArray(schema['@type'])
...
test('homepage publishes the stable business identity used by article publishers', () => {
  ...
  const business = homepageSchemas.find((schema) => schemaHasType(schema, 'CafeOrCoffeeShop'));
...
test('visible localized bylines and dates match the Article schema', () => {
  const business = jsonLd(read('../index.html')).find(
    (schema) => schemaHasType(schema, 'CafeOrCoffeeShop')
  );
